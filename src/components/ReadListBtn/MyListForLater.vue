		...
		<div :class="click ? 'style-color-event-open' : 'style-color-event-clouse'">
			...
			<div class="all-list">
				...
				<div
					...
					v-for="readlist in showMyListForLater()"
					:key="readlist.id"
				>
					<div
...
export default {
	...
	computed: {
		...mapState({
			...
			showMyReadLater: (state) => state.readLater.showMyReadLater,
		}),
	},
	methods: {
		showMyListForLater() {
			let specify = this.readLater.filter((data) => {
				return data.nameList === this.showMyReadLater;
			});
