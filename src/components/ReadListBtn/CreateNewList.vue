	...
	<div>
		<div class="wrap-btn-header">
			...
			<div class="btn-create-list">
				<p @click="eventChangeDescrip()">Save</p>
			</div>
		</div>
...
export default {
	...
	computed: {
		...mapState({
			bookForList: (state) => state.myList.bookForList,
			myList: (state) => state.myList.myList,
		}),
	},
	methods: {
		...mapMutations({ defaultDescrip: "myList/editDefaultDescriptionBook" }),
		eventChangeDescrip() {
			if (this.forDescripList == "") {
				this.forDescripList = "No description";
			}
			if (this.forNameList !== "") {
				this.defaultDescrip({
					idBook: [this.bookForList],
					nameList: this.forNameList,
					descriptionList: this.forDescripList,
				});
