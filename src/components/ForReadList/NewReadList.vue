	...
	<div>
		<div class="wrap-btn-header">
			...
			<div class="btn-create-list">
				<p @click="takeNameAndDescripForMyReadList()">
					Save
				</p>
			</div>
		</div>
...
export default {
	...
	computed: {
		...mapState({
			takeImgForShowInCreateReadList: (state) =>
				state.myList.takeImgForShowInCreateReadList,
		}),
	},
	methods: {
		...mapMutations({
			// from readlist.js
			sendPostForReadList: "myList/sendPostForReadList",
		}),
		takeNameAndDescripForMyReadList() {
			...
			if (this.forNameList !== "" && this.forDescripList !== "") {
				...
				// from readlist.js
				this.sendPostForReadList(namesListAndDescription);
