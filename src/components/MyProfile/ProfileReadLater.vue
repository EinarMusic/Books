	...
	<div>
		<div v-for="readlater in readLater" :key="readlater.id">
			<div class="content-profile-read">
				<div class="descriptions-read">
					...
					<div
						...
						@click="
							makeTheComment(),
								writeComment(),
								sendDataForPost(readlater.nameList, readlater.list)
						"
					>
						<i
				...
				<div class="some-to">
					<div
						@click="addedBookForDebate(showImg.book, readlater.nameList)"
						v-for="showImg in showMyListForRead(readlater.nameList).myBooks"
						:key="showImg.id"
					>
						<div class="show-read">
...
export default {
	...
	methods: {
		...mapActions({
			takeReadLater: "readLater/takeReadLater",
		}),
		...mapMutations({
		...
		addedBookForDebate(id, name) {
			if (!this.add.some((data) => data == id)) {
				let book = this.showMyListForLater(name);
